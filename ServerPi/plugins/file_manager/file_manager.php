			...
			while (($file = readdir($dir_opened)) !== false) {
				if( $file != '.' && $file != '..' && $file != 'index.php' && $file != 'ServerPi') {
					if(filetype($dir . $file) == "dir") {
						...
						echo '<tr id="'. $accordion_nb .'accordion" class="accordion"><td>lol</td></tr>';
						$accordion_nb++;
					}
					else {
						...
						echo '<t><td><a href="' .$_GET["dir"]. $file . '">' . $file . '</a></td><td WIDTH=81>'. $size . '</td><td width=10><a href="#'. $accordion_nb .'accordion">&#x2807;</a></td></tr>';
						echo '<div id="'. $accordion_nb .'accordion" class="accordion"><p>lol</p></div>';
						$accordion_nb++;
					}
				}
			}
